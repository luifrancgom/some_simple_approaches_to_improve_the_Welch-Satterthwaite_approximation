...
$$\widehat{\alpha}_j = F_{T}\left(t_{\alpha,\widehat{\nu}}\right) + \left(1 - F_{T}\left(t_{1-\alpha,\widehat{\nu}}\right)\right) \text{ for } j = 1, 2, \ldots, 360$$ {#eq-type-I-error-rates}

where $F_T$ represents the empirical cumulative distribution function of the two-sample t-test statistic.

[^type-I-error-ws]: The index $j$ takes on values from $1$ to $360$ because $\rho$ takes $6$, $n_1$ $10$, $k$ $3$ and $\alpha$ $2$ values. Therefore $\widehat{\alpha}_j$ can take $360$ values.
...
@tbl-sim-pro-rel-alpha-005 and @tbl-sim-pro-rel-alpha-010 present the results for the combination $(\rho, n_1, k)$ for $\alpha = 0.05$ and $\alpha = 0.10$. Also @fig-sim-pro-rel-alpha visually represents these results, demonstrating that the accuracy of the estimated type I error rates generally improves as  $n_1$ increases and $k$ decreases. However, the effect of the variance ratio, $\rho$, on accuracy is not as clear-cut and warrants further investigation.
...
data_table <- simulation_tbl_stacked |> 
